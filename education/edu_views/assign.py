...
from education.edu_forms.assigntment import AssignMentForm, CheckinAssignmentForm
...
def create_assign(request,nor_pk):
    ...
    if request.method=='POST':
        ...
        form=AssignMentForm()
    return render(request,"education/normative/create_assign.html",{'form':form})
...
def mark_assign_student(request,assign_pk):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            form.save(commit=False)
            assign.grade=form.cleaned_data.get("grade")
